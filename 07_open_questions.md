...
This is the single most contradicted area across code, SOW, and handwriting. It must be resolved first because nearly every other entity references a file by its identifier.

### Q1.1 — Is the File Number auto-generated or user-entered?
...
- [SPECIFIED] SOW §2.1 says the file cover carries a "file unique number"; §3.1 says "new file can be opened by anyone."

**CONFLICT:** Prototype (free-text) directly contradicts H1 (auto-generated). **Surface and resolve with client.**
...
### Q1.2 — What exactly is the "UN number" and how does it relate to fileNumber and the auto Sr.no?
...
**CONFLICT:** UN number is filterable/displayable but never captured at creation. Is it the same as the H1 Sr.no? Unresolved.

**Recommended default to confirm:** Treat UN number as the **system-generated unique sequence (the H1 Sr.no)**, assigned on submission, and treat `fileNumber` as the human/section-facing label. Confirm format (`UN-YYYY-NNN`?) and whether UN is global or per-year/per-section.

### Q1.3 — Uniqueness scope of each identifier
...
### Q1.4 — Note and correspondence numbering scope and "page-no.-wise on print"
...
- [SPECIFIED] SOW §2.2A/B: notes are sequential Note1→Note2; correspondence numbered C/1, C/2…; numbering cannot normally change.
...
**OPEN / UNCLEAR:** H18 wording is ambiguous — re-confirm with client (see Q13.3). Distinguish *logical* numbering (Note n / C/n) from *physical print pagination*.

**Recommended default to confirm:** Logical Note/C numbers are file-scoped, sequential, immutable once submitted, gaps preserved. Print pagination is a separate page-number sequence applied at print time. Confirm H18 interpretation.

---

## Theme 2 — File Lock ("File locked — Creator / Checker", H6)

### Q2.1 — What does a file "lock" permit and prevent?
...
- [SPECIFIED] SOW §2.2A: "after processing a note the file generally reverts to the originator" — implies single-holder-at-a-time movement.

**Recommended default to confirm:** Lock = the file is held by exactly one party at a time (the current assignee/holder), and only the holder may add notes/correspondence or forward it; everyone else has read-only view. The UI surfaces whether the current holder is the Creator or a Checker. Confirm.

### Q2.2 — Who can hold a file, and what happens to in-flight edits / drafts on transfer?
...
- [SPECIFIED] SOW §5: if a reviewer is absent, a colleague/reporting officer may take approval and "any concerned person may change the flow" if urgent — implies the holder can change.
...
### Q2.3 — Override / break-lock authority (absent reviewer case)
...
**What we know:**
- [SPECIFIED] SOW §5: absent reviewer → colleague/reporting officer can approve via mail (attach to correspondence, annotate "on leave, approval received on mail"); urgent → "any concerned person may change the flow."
- [OBSERVED] No override mechanism in code.

**Recommended default to confirm:** Allow a designated reporting officer / section head to reassign a held file, with the override recorded as a movement. Confirm who qualifies as "any concerned person."

---

## Theme 3 — Workflow & Routing Model

### Q3.1 — Fixed Maker→Checker→Approver chain, or free recipient list?
...
**What we know:**
- [SPECIFIED] S6 names a "Maker-Checker-Approver workflow"; SOW §3 names Originator + Reviewer/Approver only (two role types, not four).
...
**CONFLICT/TENSION:** Data implies a typed role chain; the only UI implements a free list. SOW wants a *predefined-but-modifiable* chain — a hybrid. Resolve the canonical model.

**Recommended default to confirm:** Hybrid — a **predefined Maker→Checker(s)→Approver(/MD) chain** that the holder can modify mid-flow (add/remove/reorder recipients). Confirm the canonical role set (see Q4.2 re MD vs Approver).

### Q3.2 — How is the "pre-defined recipient workflow" defined, and when?
...
**Why it matters:** S18 + SOW §3.1 say the originator "plans the initial flow" *at creation*. The prototype has no routing at creation.

**What we know:**
- [SPECIFIED] SOW §3.1 / §5: originator plans the initial flow and adds the initial recipient list **on the note**; reviewers can add further members.
...
**CONFLICT:** SOW wants initial-flow planning at creation; prototype only offers it at forward time. Surface.

**Recommended default to confirm:** Capture the initial recipient chain at creation (originator-planned), allow any subsequent holder to add checkers/recipients (H13). Confirm whether section templates provide a default chain.

### Q3.3 — Sequential vs parallel checkers (S6a, multiple checker support)
...
- [SPECIFIED] SOW §2.2A: file moves note-by-note and "generally reverts to originator" — implies sequential handling.

**Recommended default to confirm:** Sequential by default (ordered chain, single holder at a time — consistent with the lock model), with parallel as a possible future option. Confirm whether true parallel review is required.

### Q3.4 — "Commenting AFTER checker approval" (S6b) semantics
...
**Recommended default to confirm:** Post-approval comments are **append-only annotations** that do not erase the prior approval but are recorded in the audit trail; a subsequent revert is a separate explicit action. Confirm whether a post-approval comment can itself trigger revert.

### Q3.5 — The 6 review actions vs the canonical 3
...
- [OBSERVED] ReviewModal renders **6** actions: Check, Approve, Revert, Approve with Conditions, Reject, Request Clarification (`src/components/ReviewModal.jsx:72-113`), exceeding spec.
- [SPECIFIED] SOW §3.3/4.3 lists richer remarks (Approval, Correction, Rejection, Clarification request, Additional-docs request, Return) — supports the larger set conceptually.

**TENSION:** SOW §4.3 supports 6, but S6c/handwriting name only 3 as canonical. Resolve the authoritative action set.

**Recommended default to confirm:** Canonical 3 (Check/Approve/Revert) as the enforced state transitions; Reject / Request Clarification / Approve-with-Conditions modeled as variants of Revert/Approve. Each action records timestamp + department (H5). Confirm.

### Q3.6 — Paragraph-level (partial) approval
**Question:** Is paragraph-level approval ("approve para A but not B", marker A/B/C) in scope for Phase 1?

**Why it matters:** It complicates the note/approval data model and the print-of-approval rendering.

**What we know:**
- [SPECIFIED] SOW §4.2: approval/remarks "may be recorded against a SPECIFIC PARAGRAPH" marked 'A'.
...
**Recommended default to confirm:** In scope conceptually; confirm whether Phase 1 needs full paragraph-level approval or only whole-note approval, given the print/signature implications.

---

## Theme 4 — Roles & Access Control

### Q4.1 — Who can open (create) a file?
...
**What we know:**
- [SPECIFIED] SOW §3.1 explicitly: "**new file can be opened by anyone**."
- [OBSERVED] No gating in code; any visitor reaches CreateFile (`src/App.jsx` route `/create-file`; no auth guard).

**Recommended default to confirm:** Any authenticated user may create a file (per SOW). Confirm whether section membership is still required.

### Q4.2 — MD vs Approver distinction
...
- [OBSERVED] ReviewModal has a dedicated "MD Approval (Offline)" upload section (`src/components/ReviewModal.jsx:150-185`) — **and this path crashes** because `<FiUpload/>` (`:174`) is rendered but not imported (imports at `src/components/ReviewModal.jsx:2` are FiX, FiCheck, FiXCircle, FiArrowLeft, FiSend). Helper text says "Any maker or checker can upload physical scanned approval" (`:181`).
- [OBSERVED] No role model distinguishes MD; `approver` is a single embedded {id,name} (`src/data/dummyData.js:50`).

**Recommended default to confirm:** MD = a distinguished final-approver role whose approval can be recorded as an uploaded scanned document (offline path). Confirm whether MD is always the terminal step and whether non-MD final approvers also exist.

### Q4.3 — Section-level edit rights (S17, H11/H12)
...
**Recommended default to confirm:** Edit rights scoped to a user's own section and role within the active workflow step; cross-section users are read-only unless the file is routed to them. Confirm exact granularity with the requester (Rasika).

### Q4.4 — Confidential file access (SOW §7) — which parts Phase 1 vs Phase 2?
...
**What we know:**
- [SPECIFIED] SOW §7: restricted access, secure storage, marked CONFIDENTIAL, movement only with signature; temp-file creation+merge for simultaneous use is explicitly **"2nd PHASE"** for digital.
...
**Recommended default to confirm:** Phase 1 = restrict view/open of confidential files to authorized roles + record signed movement; Phase 2 = temp-file split/merge (per SOW §7). Confirm the Phase-1 authorization rule for confidential files.

---

## Theme 5 — Audit, Immutability & Movement Log

### Q5.1 — What is editable vs permanently locked?
...
**What we know:**
- [SPECIFIED] SOW §2.2A: "old notes remain permanently for audit"; numbering cannot normally change (gaps indicate deletions).
- [OBSERVED] Nothing persists or mutates today (all stubs), so no immutability is enforced. `isDraft` exists on notes (`src/data/dummyData.js:79, :107`) implying draft-vs-submitted distinction.
- [CLARIFIED] H2 hints at a "Request to edit/add" mechanism on drafts/files (wording partly unclear — see Q13.1).

**Recommended default to confirm:** Submitted notes are append-only/immutable; drafts are freely editable by their owner; file cover edits after creation require an explicit (logged) change, possibly via the H2 "request" mechanism. Confirm.

### Q5.2 — Movement-log completeness
...
**What we know:**
- [SPECIFIED] SOW §5: "ALL MOVEMENTS ARE RECORDED ensuring traceability from start to closure"; S10 wants "complete file logs."
...
**Recommended default to confirm:** Log every state-changing action (create, forward, check, approve, revert, return, reassign, transfer, close) with timestamp + actor + department (H5). Confirm the full event taxonomy.

### Q5.3 — Export of logs/reports (H8)
...
**Recommended default to confirm:** Phase 1 = export the audit/report log to CSV (and ideally PDF). Confirm formats and scope.

---

## Theme 6 — Digital Signature (S13)

### Q6.1 — What is the signature mechanism: real PKI, typed name, or image?
...
- [CLARIFIED] H9 groups S11/S12/S13 with the S14 approval-on-print bundle — signature is tied to the printed-approval details.

**Recommended default to confirm:** Phase 1 = typed-name + system attestation (user, timestamp, location captured automatically), rendered on print; real PKI/DSC certificate signing deferred unless legally required. **Confirm legal-weight requirement explicitly** — it changes everything.

### Q6.2 — What metadata must a signature capture (esp. "Location", S14)?
...
**Recommended default to confirm:** "Location" = the actor's section/office (per H5 "dept."), not GPS. Confirm.

---

## Theme 7 — Correspondence Enhancements

### Q7.1 — Page-level linking ("C36 on page 5", S20b / H15) — exact behavior
...
### Q7.2 — Email references / email-attachment integration (S20a, H15)
...
- [OBSERVED] AddCorrespondenceModal offers an "Email Reference" upload-type radio and an Email Reference text field (`src/components/AddCorrespondenceModal.jsx:138-207`); discarded on submit. SOW §5 describes attaching an approval *mail* as correspondence when a reviewer is absent.

**Recommended default to confirm:** Phase 1 = attach an email as a correspondence document (upload or reference string), no live mailbox sync. Confirm.

### Q7.3 — Drag-and-drop attachment behavior
...
### Q7.4 — Supported file formats (S5) — confirmed Phase 2
...
**Recommended default to confirm:** Full multi-format attachment + preview = Phase 2 (per H4). Phase 1 may store a file reference/placeholder only. Confirm the minimal Phase-1 attachment capability.

---

## Theme 8 — Print

### Q8.1 — Page-range selection + summary page (S16, H11)
...
### Q8.2 — Approval details on printed documents (S14)
...
**Recommended default to confirm:** Print renders all actors in the chain with sign + timestamp + approved-by + section/location. Confirm exact layout.

### Q8.3 — Header/footer content (H16)
...
**Recommended default to confirm:** Header/footer carries file name + Period + UN number (per H16). Confirm whether this is print-only or also on-screen.

### Q8.4 — Print page numbering "page-no.-wise" (H18)
**Question:** See Q1.4 — confirm that print applies a physical page-number sequence distinct from logical Note/C numbering.

**What we know:** [CLARIFIED] H18 (flagged unclear). Re-confirm wording (Q13.3).

**Recommended default to confirm:** Print assigns sequential physical page numbers across the printed range. Confirm.

---

## Theme 9 — Closure & Lifecycle

### Q9.1 — File closure & successor-file linkage (SOW §9, H17)
...
**What we know:**
- [SPECIFIED] SOW §9: a file is closed when full; closing date + reason + successor file's unique number recorded; a new file opens in its place; start/end period + complete movement history remain permanent.
- [CLARIFIED] H17: "**File close date**" — capture/display closure date.
- [OBSERVED] Status `'Closed'` never appears in data (only Open/Under Review/Approved) (`src/data/dummyData.js:36` across files); `endPeriod` always null (`:45`); no closure UI (orchestrator-verified ABSENT). Dashboard even treats closure inconsistently (overdue logic excludes 'Closed' that never exists, `src/pages/Dashboard.jsx:11-14`).

**Recommended default to confirm:** Closure captures close date + reason + successor file link (UN number), sets status to Closed, makes the file read-only, retains full history. Confirm whether the successor file is auto-created or manually linked.

### Q9.2 — Cross-department permanent transfer
...
**Why it matters:** SOW calls this out specifically (Recovery→Legal example); it differs from routing (the file's home section changes, not just its current holder).

**What we know:**
- [SPECIFIED] SOW "Additional Points": a file can be PERMANENTLY TRANSFERRED across departments; on activity transfer, related files move to the receiving dept.
- [OBSERVED] No transfer UI (orchestrator-verified ABSENT); only forward exists, which doesn't change `section`.

**Recommended default to confirm:** Model permanent transfer as a distinct action that updates the file's owning `section` (and possibly fileNumber prefix?) and is logged as a movement. Confirm whether the file number changes on transfer.

### Q9.3 — Suo-moto notes (SOW §8)
...
**What we know:**
- [SPECIFIED] SOW §8: internal notes recorded WITHOUT correspondence (policy/admin decisions/internal reviews).
...
**Recommended default to confirm:** Suo-moto = a normal note with no correspondence references (implicit), optionally flagged for filtering. Confirm whether an explicit flag/category is needed.

---

## Theme 10 — Inbox Semantics (Inward / Outward, S7)

### Q10.1 — Exact rules distinguishing Inward (Final Approved) vs Outward (Revision)
...
**Recommended default to confirm:** Inward = file has reached final approval and is in your queue for action/awareness; Outward = file reverted to you for revision/rework. Each may expose different default actions (e.g. Outward → Add Note/resubmit; Inward → acknowledge/route). Confirm whether `inboxType` is a stored flag or derived from status + last action.

### Q10.2 — Is inboxType stored or derived?
**Question:** Should Inward/Outward be a persisted field or computed from the workflow state?

**What we know:** [OBSERVED] currently a stored field on File (`src/data/dummyData.js:51`).
...
### Q11.1 — DMS / old-file retrieval (S15) — confirmed Phase 2 scope and boundary
...
### Q11.2 — Relationship to the existing separate File Tracking system
**Question:** Does this app integrate with, replace, or merely coexist with the existing File Tracking app? Is data shared?

**Why it matters:** SOW says a working File Tracking app already exists for demonstrating tracking + reports. Overlap with our movement log / Reports must be clarified to avoid duplication.

**What we know:**
- [SPECIFIED] SOW "Additional Points": there is ALSO a SEPARATE FILE TRACKING system (a working app exists) that tracks file flow org-wide and provides reports.
- [OBSERVED] This prototype has its own movement log + Reports page (`src/pages/Reports.jsx`).

**Recommended default to confirm:** This app owns the N-C file/noting/approval workflow; the separate File Tracking system owns org-wide flow tracking. Define the integration boundary (data feed? shared IDs? none?). **Confirm with client — this is a key architectural boundary.**

---

## Theme 12 — Mobile (S21, H19)

### Q12.1 — Mobile scope and feature parity
**Question:** Is mobile a responsive web build or a native app, and which features must reach parity (view-only vs full approval workflow on mobile)?

**Why it matters:** Native app + full workflow parity is a separate delivery stream; responsive web is far smaller.

**What we know:**
- [SPECIFIED] S21: "Mobile application development."
- [CLARIFIED] H19 confirms S21 is IN SCOPE.
- [OBSERVED] No mobile app; possibly responsive CSS only (orchestrator lists mobile app ABSENT).

**Recommended default to confirm:** Phase 1 = responsive web for view + approve/forward; native app deferred. **Confirm native-vs-responsive and which actions are mobile-critical** (likely approvals).

---

## Theme 13 — Unclear Handwriting to Re-Confirm With Client

These items have low-confidence transcriptions and must be verified verbatim with the client before they drive any design.

### Q13.1 — H2: draft "Request to edit/add"
**Question:** What is the "Request" mechanism on drafts/files (S3)? A workflow to request edit/add rights from a holder/approver?

**What we know:** [CLARIFIED] H2 on S3: a note about "file name" and a "Request" that can be sent to "edit/add" (wording partly unclear). Relates to lock (Theme 2) and immutability (Q5.1).

**Action:** Re-confirm exact wording and intent with client.

### Q13.2 — H4 trailing text on S5
**Question:** What is the trailing "9/w..." annotation next to the "2nd phase" mark on S5?

**What we know:** [CLARIFIED] H4: S5 marked Phase 2; trailing fragment unclear.

**Action:** Re-confirm; likely a date or initials.

### Q13.3 — H18 numbering semantics
**Question:** Confirm meaning of "any number → upto submit; print → page-no.-wise" (Q1.4 / Q8.4).

**What we know:** [CLARIFIED] H18 flagged unclear by transcriber.

**Action:** Re-confirm whether this is about provisional logical numbering vs physical print pagination.

### Q13.4 — H20 circled emphasis marks
...
### Q14.1 — Post-approval routing to actionable dept and return to Maker
...
**Recommended default to confirm:** On final approval, the holder can print + attach the approved file and route to an actionable department; that department adds comments; the file then returns to the Maker. Model this as explicit post-approval states. Confirm the exact step sequence.
...
- **P7 — `endPeriod` bound in form but missing from initial state** (`src/pages/CreateFile.jsx:9-17` vs `:118`); works only by dynamic key assignment. Relevant to file-cover model + closure (Q9.1, which sets end period).
...
The following are our recommended Phase-2 deferrals based on [CLARIFIED] handwriting and [SPECIFIED] SOW. **Confirm each with the client** — these are proposals, not decisions.
...
| O3 | Confidential temp-file split & merge for simultaneous use — SOW §7 | [SPECIFIED] "2nd PHASE" for digital | High |
| O4 | Real PKI / DSC certificate digital signatures (vs typed-name attestation) — S13 | [SPECIFIED] S13 "if required"; [OBSERVED] only text input | Medium — depends on legal-weight answer (Q6.1) |
...
| O6 | Live email-mailbox integration (vs manual email attachment) — S20a | [INFERRED] manual attach is Phase 1; live sync is heavier | Medium |
| O7 | Native mobile application (vs responsive web) — S21 | [CLARIFIED] H19 confirms scope, but [INFERRED] native deferred | Medium — confirm native-vs-responsive (Q12.1) |
| O8 | Paragraph-level conditional approval, if it complicates Phase 1 — SOW §4.2 | [INFERRED] may defer given print/signature complexity | Low — client may want it in Phase 1 |
| O9 | Notifications (in-app/email/SMS/push) | [OBSERVED] absent; doc-only claim | Medium — confirm |

**Explicitly IN scope for Phase 1 (everything not deferred above), notably:** UN/dept display (S1), draft notes (S3), approved-file search/reference (S4), Maker-Checker-Approver workflow incl. multiple checkers and post-checker comments (S6), Inward/Outward inbox (S7), created-by visibility (S8), two-page Noting/Correspondence preview (S9), reports + export (S10/H8), MD approval incl. offline upload (S11), final-approval print with page-range + summary + approval details + header/footer (S12/S14/S16/H16), section-level edit rights (S17), predefined-but-modifiable recipient workflow (S18/H13), post-approval routing back to Maker (S19/H14), file lock concept (H6), closure date + successor linkage (S/H17), cross-dept permanent transfer, and suo-moto notes — **all subject to the decisions above.**

> **Note on the two-page preview orientation (S9 / H7):** [CLARIFIED] H7 maps **L = Noting Side / R = Correspondence Side** (left/right), matching the physical N-C file. [OBSERVED] the prototype actually **stacks the two sides vertically** — NOTING SIDE is the "Top Half" (`src/pages/FileDetail.jsx:349-352`) and CORRESPONDENCE SIDE the "Bottom Half" (`:406-409`) — not side-by-side. The SOW author-margin convention (Note-1 author at right margin, later notes at left) is still honored; only the pane arrangement differs (top/bottom in code vs left/right in H7). Confirm the intended Phase-1 layout (left/right vs top/bottom). This does not change any decision above; it is recorded so the left/right framing elsewhere in the discovery set is read against the current code reality.
